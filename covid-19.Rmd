...
corona <- corona %>% mutate(state_code = map_chr(fips, ~str_extract(., '\\d{2}')))
...
```{r Pennsylvania}
map_plot <- animate_states("42", title = 'Pennsylania')
...
line_plot <- corona %>% 
  ...
  geom_line(size = 1.2, color = '#e01a1a') +
  labs(y = 'Total Confirmed Cases', x = 'Date',
       title = 'Confirmed Cases of Coronavirus in Pennsylvania',
...
line_anim <- animate(line_plot, end_pause = 20)

grid.arrange(map_plot, line_anim, nrow = 2,
                        top = grid::textGrob("Confirmed Cases of Covid-19 in Pennsylvania by County",
                                             gp=grid::gpar(fontsize=16, font=2)))

map_plot
anim_save(filename = 'plots/pa_1.gif', animation = map_plot)
line_anim
anim_save(filename = 'plots/pa_2.gif', animation = line_anim)

# ggsave(filename = 'plots/PA_test.png', width = 8, height = 5)
```
